...
#Create graphs
for i in range(0, 4):
	print("\t\\addplot[only marks, mark size=1.5pt, color=" + colors[i%len(colors)] + ", mark=" + marks[i%len(marks)] + "] plot coordinates {")
	
	for j in range(0, csv_file.shape[0]):
		if pd.isna(csv_file.iat[j, i]) == False:
			if isinstance(csv_file.iat[j, i], numbers.Number):
				print("\t\t(" + str(csv_file.iat[j, i*2]) + ", " + str(csv_file.iat[j, i*2+1]) + ")")
