...
def select_columns(df):
	cols_to_keep = ['rwl_est_frict_torque', 'rwl_frict_coeff', 'rwl_meas_ang_mom', 'rwl_wheel_direction', 'rwl_wheel_speed']
...
def normalize_cnt(df):
	df = df.copy()
	df['rwl_est_frict_torque'] = df['rwl_est_frict_torque'] / df['rwl_est_frict_torque'].max()
	return df
	
dataset = (df.pipe(select_columns).pipe(onehot_encode_integers, excluded_cols=['rwl_est_frict_torque']).pipe(normalize_cnt))
...
val_time = 30000 #int(len(df)*0.8)
test_time = 36000 #int(len(df)*0.9)

window_len = 24 * 3
forecast_len = 24

n_total_features = len(dataset.columns)
n_aleatoric_features = len(['rwl_est_frict_torque', 'rwl_frict_coeff', 'rwl_meas_ang_mom', ])
#['rwl_wheel_direction', 'rwl_wheel_speed']
...
AIHelp.plot_learning(filename + " - RWL Single Shot", history)
